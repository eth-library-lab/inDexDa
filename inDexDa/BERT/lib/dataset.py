...
class Dataset(data.Dataset):
    ...
    def get_dataset(self):
        dataset = []
        with open('/home/parker/code/datadex/inDexDa/BERT/log/dataset.json', 'r') as f:
            contents = f.read()
            try:
    ...
    def json_to_tsv(self):
        ...
        if self.prepare_tsv:
            if self.train:
                data_df.to_csv('../log/train.tsv', sep='\t', index=False, header=False)
            else:
                data_df.to_csv('../log/test.tsv', sep='\t', index=False, header=False)
